...
class Services_Qiniu_Storage_ResumeUploader
{
    private $upToken;
    private $key;
    private $inputStream;
    ...
    public function upload()
    {
        ...
        while ($uploaded < $this->size) {
            ...
            if (! $response->ok() || !isset($ret['crc32'])|| $crc != $ret['crc32']) {
                fclose($this->inputStream);
                return array(null, new Services_Qiniu_Http_Error($this->currentUrl, $response));
            }
    ...
    private function post($url, $data)
    {
        ...
        return Services_Qiniu_Http_Client::post($url, $data, $headers);
    }

    private function blockSize($uploaded)
    {
        if ($this->size < $uploaded + Services_Qiniu_Config::BLOCK_SIZE) {
            return $this->size - $uploaded;
        }
        return  Services_Qiniu_Config::BLOCK_SIZE;
    }
}
