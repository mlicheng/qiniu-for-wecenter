...
class Services_Qiniu_Storage_BucketManager
{
    private $auth;

    public function __construct(Services_Qiniu_Auth $auth)
    {
        $this->auth = $auth;
    }
    ...
    public function listFiles($bucket, $prefix = null, $marker = null, $limit = 1000, $delimiter = null)
    {
        ...
        $url = Services_Qiniu_Config::RSF_HOST . '/list?' . http_build_query($query);
    ...
    {
        $url = Services_Qiniu_Config::RS_HOST . $path;
    ...
    {
        $url = Services_Qiniu_Config::IO_HOST . $path;
        return $this->post($url, $body);
    }
    ...
    private function get($url)
    {
        ...
        $ret = Services_Qiniu_Http_Client::get($url, $headers);
        if (!$ret->ok()) {
            return array(null, new Services_Qiniu_Http_Error($url, $ret));
        }
    ...
    private function post($url, $body)
    {
        ...
        $ret = Services_Qiniu_Http_Client::post($url, $body, $headers);
        if (!$ret->ok()) {
            return array(null, new Services_Qiniu_Http_Error($url, $ret));
        }
