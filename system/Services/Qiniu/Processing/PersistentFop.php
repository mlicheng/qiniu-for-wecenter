...
class Services_Qiniu_Processing_PersistentFop
{
    /**
     * @var 账号管理密钥对，Auth对象
     */
    ...
    public function execute($key, $fops)
    {
        ...
        $url = Services_Qiniu_Config::API_HOST . '/pfop/';
        $headers = $this->auth->authorization($url, $data, 'application/x-www-form-urlencoded');
        $headers['Content-Type'] = 'application/x-www-form-urlencoded';
        $response = Services_Qiniu_Http_Client::post($url, $data, $headers);
        if (!$response->ok()) {
            return array(null, new Services_Qiniu_Http_Error($url, $response));
        }
        $r = $response->json();
        $id = $r['persistentId'];
    ...
    {
        $url = Services_Qiniu_Config::API_HOST . "/status/get/prefop?id=$id";
        $response = Services_Qiniu_Http_Client::get($url);
        if (!$response->ok()) {
            return array(null, new Services_Qiniu_Http_Error($url, $response));
        }
